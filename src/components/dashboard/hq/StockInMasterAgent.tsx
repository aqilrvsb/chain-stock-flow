...
const StockInMasterAgent = () => {
  ...
  const { data: masterAgentStats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // For each master agent, get their inventory and transactions from HQ
      const stats = await Promise.all(
        masterAgents.map(async (ma) => {
          ...
          // Get transactions where master agent bought from HQ
          let query = supabase
            .from("transactions")
            .select(`
              *,
          ...
          const inventoryWithTransactions = (inventory || []).map(inv => {
            ...
            const totalPurchased = relatedTransactions.reduce((sum, tx) => sum + tx.quantity, 0);
            const totalCost = relatedTransactions.reduce((sum, tx) => sum + parseFloat(String(tx.total_price)), 0);
...
export default StockInMasterAgent;
