...
const JENIS_CLOSING_OPTIONS = ["Website", "WhatsappBot", "Manual", "Call", "Live", "Shop", "Walk In"];
...
const BranchSpend = () => {
  ...
  // Fetch spends for this branch
  const { data: spends = [], isLoading } = useQuery({
    queryKey: ["branch-spends", user?.id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("spends")
        .select("*")
        .eq("branch_id", user?.id)
        .is("marketer_id_staff", null) // Only get branch's own spends, not marketer spends
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      if (editingSpend) {
        ...
        const { error } = await supabase.from("spends").insert({
          ...
          total_spend: parseFloat(data.totalSpend),
          tarikh_spend: data.tarikhSpend,
          branch_id: user?.id, // Link to branch
        });
        if (error) throw error;
      }
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["branch-spends"] });
      handleCloseDialog();
    },
    onError: (error: any) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-primary">Branch Spend</h1>
          <p className="text-muted-foreground">Manage your branch marketing spend</p>
        </div>
        <Dialog
          open={isDialogOpen}
          onOpenChange={(open) => {
        ...
        >
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="tarikhSpend">Spend Date *</Label>
                <Input
                  id="tarikhSpend"
                  type="date"
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Spend?</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete this spend? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
