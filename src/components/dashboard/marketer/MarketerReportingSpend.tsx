...
const MarketerReportingSpend = () => {
  ...
  const { data: spends = [], isLoading: spendsLoading } = useQuery({
    queryKey: ["marketer-reporting-spends", userIdStaff],
    queryFn: async () => {
  ...
  const { data: prospects = [], isLoading: prospectsLoading } = useQuery({
    queryKey: ["marketer-reporting-prospects", userIdStaff],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("prospects")
        .select("*")
  ...
  const { data: orders = [], isLoading: ordersLoading } = useQuery({
    queryKey: ["marketer-reporting-orders", userIdStaff],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const aggregatedData = useMemo(() => {
    ...
    dataMap.forEach((value) => {
      ...
      const platformOrders = filteredOrders.filter((o: any) => o.produk === value.product && o.jenis_platform === value.platform);
