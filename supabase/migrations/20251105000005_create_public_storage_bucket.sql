...
USING (
  ...
  EXISTS (
    SELECT 1 FROM public.master_agent_relationships mar
    INNER JOIN public.agent_purchases ap ON ap.agent_id = mar.agent_id
    WHERE mar.master_agent_id = auth.uid()
    AND ap.receipt_image_url LIKE '%' || (storage.objects.name) || '%'
  )
);
