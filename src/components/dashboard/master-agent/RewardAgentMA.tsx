...
const RewardAgentMA = () => {
  ...
  const { data: rewardData, isLoading } = useQuery({
    queryKey: ["reward-agent-ma-progress", user?.id, periodFilter, selectedMonth, selectedYear],
    queryFn: async () => {
      ...
      // Get transactions for each agent (purchases from this Master Agent)
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-4xl font-bold bg-gradient-to-r from-primary to-blue-600 bg-clip-text text-transparent">
          My Agents Reward Progress
        </h1>
        <p className="text-muted-foreground mt-2">
          Track your agents' performance towards monthly reward targets
        </p>
      </div>
