...
const RewardAgentGold = () => {
  ...
  const { data: rewardData, isLoading } = useQuery({
    queryKey: ["reward-agent-gold-ma-progress", user?.id, periodFilter, selectedMonth, selectedYear],
    queryFn: async () => {
      const month = periodFilter === "monthly" ? parseInt(selectedMonth) : 0;
      const year = parseInt(selectedYear);

      // Get active rewards for gold agents
      const { data: rewards } = await supabase
        .from("rewards_config")
        .select("*")
        .eq("role_subrole", "gold")
        .eq("month", month)
        .eq("year", year)
        .eq("is_active", true)
        .order("month", { ascending: false });

      // Get only THIS master agent's gold agents via relationships table
      ...
      // Filter for gold agents only
      const agents = relationships
        ?.map(rel => rel.agent)
        .filter(agent => agent?.sub_role === 'gold') || [];
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-4xl font-bold bg-gradient-to-r from-primary to-blue-600 bg-clip-text text-transparent">
          Reward Agent (Gold) Progress
        </h1>
        <p className="text-muted-foreground mt-2">
          Track your Gold agents' performance towards reward targets
        </p>
      </div>
            ...
            <div className="space-y-2">
              ...
              <RadioGroup value={periodFilter} onValueChange={(value: any) => setPeriodFilter(value)} className="flex gap-4">
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="monthly" id="agent-gold-ma-filter-monthly" />
                  <Label htmlFor="agent-gold-ma-filter-monthly" className="font-normal cursor-pointer">Monthly</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="yearly" id="agent-gold-ma-filter-yearly" />
                  <Label htmlFor="agent-gold-ma-filter-yearly" className="font-normal cursor-pointer">Yearly</Label>
                </div>
              </RadioGroup>
            </div>
...
export default RewardAgentGold;
