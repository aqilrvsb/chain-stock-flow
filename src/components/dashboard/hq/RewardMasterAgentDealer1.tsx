...
const RewardMasterAgentDealer1 = () => {
  ...
  const { data: rewardData, isLoading } = useQuery({
    queryKey: ["reward-ma-dealer1-progress", periodFilter, selectedMonth, selectedYear],
    queryFn: async () => {
      const month = periodFilter === "monthly" ? parseInt(selectedMonth) : 0;
      const year = parseInt(selectedYear);

      // Get active rewards for dealer_1 master agents
      const { data: rewards } = await supabase
        .from("rewards_config")
        .select("*")
        .eq("role_subrole", "dealer_1")
        .eq("month", month)
        .eq("year", year)
        .eq("is_active", true)
        .order("month", { ascending: false });

      // Get all dealer_1 master agents
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-4xl font-bold bg-gradient-to-r from-primary to-blue-600 bg-clip-text text-transparent">
          Reward Master Agent (Dealer 1) Progress
        </h1>
        <p className="text-muted-foreground mt-2">
          Track Dealer 1 Master Agent performance towards reward targets
        </p>
      </div>
...
export default RewardMasterAgentDealer1;
