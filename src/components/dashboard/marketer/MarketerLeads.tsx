...
const MarketerLeads = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      if (editingProspect) {
        ...
        const { error } = await supabase.from("prospects").insert({
          ...
          no_telefon: data.noTelefon,
          niche: data.niche,
          jenis_prospek: "", // Will be auto-determined by OrderForm based on lead date
          tarikh_phone_number: data.tarikhPhoneNumber,
    ...
    onSuccess: () => {
      toast.success(editingProspect ? "Prospect Dikemaskini" : "Prospect Ditambah");
      queryClient.invalidateQueries({ queryKey: ["marketer-prospects"] });
      handleCloseDialog();
    },
    onError: (error: any) => {
      toast.error(error.message || "Gagal menyimpan prospect");
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Prospect Dipadam");
      queryClient.invalidateQueries({ queryKey: ["marketer-prospects"] });
    },
    onError: () => {
      toast.error("Gagal memadam prospect");
    },
  });
  ...
  const handleViewOrders = async (prospect: any) => {
    ...
    try {
      const { data: orders, error } = await supabase
        ...
        .eq("marketer_id_staff", prospect.marketer_id_staff)
      ...
      toast.error("Gagal mendapatkan senarai order.");
  ...
  const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type) && !file.name.endsWith(".csv") && !file.name.endsWith(".xlsx") && !file.name.endsWith(".xls")) {
      toast.error("Sila muat naik fail Excel (.xlsx, .xls) atau CSV.");
      return;
    }
    ...
    try {
      ...
      if (lines.length < 2) {
        toast.error("Fail tidak mengandungi data.");
        return;
      }
      ...
      toast.success(`${successCount} prospect berjaya diimport. ${errorCount} gagal.`);
      queryClient.invalidateQueries({ queryKey: ["marketer-prospects"] });
    } catch (error) {
      console.error("Import error:", error);
      toast.error("Gagal mengimport fail.");
    } finally {
  ...
  const exportCSV = () => {
    const headers = ["No", "Tarikh", "Nama", "Phone", "Niche", "Profile", "Jenis Prospek", "Count Order", "Status", "Price"];
    ...
    a.href = url;
    a.download = `prospects_${userIdStaff}.csv`;
    a.click();
    ...
    <div className="space-y-6 animate-fade-in">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.niche} onValueChange={(value) => handleChange("niche", value)}>
                    <SelectTrigger>
                      <SelectValue placeholder="Pilih product">{formData.niche || "Pilih product"}</SelectValue>
                    </SelectTrigger>
                    <SelectContent>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center flex-wrap">
        ...
        <div className="relative flex-1 max-w-xs">
          ...
          <Input placeholder="Cari nama, phone, niche..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="text-xs text-muted-foreground space-y-1">
              ...
              <ul className="list-disc pl-4 space-y-1">
                <li>
                  <strong>Nama</strong> - Nama prospek (wajib)
                </li>
                <li>
                  <strong>Telefon</strong> - No. telefon, mesti bermula dengan 6 (wajib)
                </li>
                <li>
                  <strong>Niche</strong> - Nama produk dari senarai Product (wajib)
                </li>
                <li>
                  <strong>Tarikh</strong> - Format: YYYY-MM-DD (wajib)
                </li>
              </ul>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            {isLoadingOrders ? (
              ...
              <div className="text-center py-8 text-muted-foreground">Tiada order dijumpai.</div>
            )}
          </div>
          <DialogFooter>
